...
class Button:
    ...
    def __init__(self, win, center, width, height, label):
        ...
        self.words.draw(win)
        self.activate()
        self.entrybox=Entry(Point(self.x,self.y),1)
    # a method called activate to make sure the button looks "acticated"
    #letting the user know they can click the button

    def drawlabel(self,win,text): #replace the previous label
        self.words.setText(text)

    def returnText(self): #return the label in the button
    ...
    def setColor(self,color): #change the color of the button
        self.rect.setFill(color)

        

    def entryobject(self,win): # create an entry box in the button
        self.win=win
        self.entrybox.draw(win)
        
        
    def getentryText(self): # get the text in the entry box
        self.usertext=self.entrybox.getText()
